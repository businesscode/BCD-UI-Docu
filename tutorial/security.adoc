...
To set up users and their authorization, you can choose the simple setup by hard-coding them in web.xml, by maintaining them in BCD-UI's database or authenticated with oAuth (AD, Google, facebook etc) combined with rights maintained in your database.

==== Page access rights
No matter which method you use for defining users and their roles, use the [urls] section in web.xml to define access to pages. It is easy to also adjust the menu to offer only allowed pages.
...
Instead of having a static set of users and passwords in web.xml, you can store users and passwords in the database.
...
Authentication is handled by OAuth, permissions are managed by application through _SsubjectSettings_.

. Prerequisites
.. Register your client application to Azure/Google/x to obtain a client and a client secret. For Google we use 'email' scope and for Azure 'https://graph.microsoft.com/user.read' but you can override and use whatever you prefer to use for authentication.

. Configuration
The web.xml https://github.com/businesscode/BCD-UI/blob/master/Server/configFiles/web.xml[template of BCD-UI] describes the setup, look for the following section
+
.oAuth sample with Azure
[source]
----
include::../web/WEB-INF/web.xml[tag=oAuth]
----
That web.xml also holds description for oAuth with Google and combinations for multiple providers,

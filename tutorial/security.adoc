...
To  set up users and their authorization, you can choose the simple setup by hard-coding them in web.xml
...
OAuth integration is currently limited to authentication only, the permissions for authorization are managed by application through subjectsettings.
...
Provided configurations will authenticate user and set the principal to user's email.

. Sample configuration for Azure
+
in your web.xml, Shiro configuration
+
[source]
----
[main]
...
..
----
. Sample configuration for multiple authenticators
+
in your web.xml, Shiro configuration
+
[source]

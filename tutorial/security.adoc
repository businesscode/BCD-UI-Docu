...
==== Page access rights
No matter which method you use for defining users and their roles, use the [urls] section in web.xml to define access permissions needed for pages. It is easy to also adjust the menu to offer only allowed pages.
...
. Configuration
+
The web.xml link:https://github.com/businesscode/BCD-UI/blob/master/Server/configFiles/web.xml[template of BCD-UI] describes the setup, more about this the following section
+
.oAuth sample with Azure
[source]
----
include::../web/WEB-INF/web.xml[tag=oAuth,indent=0]
----
The mentioned template web.xml also describes oAuth with Google and how to combine multiple providers,

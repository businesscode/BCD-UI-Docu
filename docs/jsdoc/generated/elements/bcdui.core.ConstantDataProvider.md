...
A data provider for constant values. This is especially useful to set values for the xsl:param elements of a [TransformationChain](bcdui.core.TransformationChain.md) subclass.
...
|----------|----------|----------|-------------|
| args | object |  | paramater map |
| args.id? | string |  | Globally unique id for use in declarative contexts |
| args.name? | string |  | The name of the data provider. This name should be unique within the scopt it is used, however it is not required to globally unique |
| args.value | (string\|number\|boolean\|object) |  | The data |
...
**Returns** {*}: The data provided in the constructor of this instance.


### getReadyStatus
getReadyStatus() &#x21FE; {bcdui.core.status.NullStatus}
...
**Returns** {bcdui.core.status.NullStatus}: The null status because since the value is provided inthe constructor and is therefore always ready.
...
**Returns** {string}: A summary of this class showing the key and value.

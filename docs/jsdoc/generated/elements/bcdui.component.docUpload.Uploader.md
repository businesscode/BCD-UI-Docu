# Class Uploader
package bcdui.component.docUpload
...
| args.scope | string |  | The scope identifier, can be a space separated string when you want to map multiple scopeId and instanceId |
| args.instance | string |  | The instance identifier, can be a space separated string when you want to map multiple scopeId and instanceId |
| args.id? | string |  | The object's id, needed only when later accessing via id. If given the docUpload registers itself at [bcdui.factory.objectRegistry](bcdui.factory.objectRegistry.md) |
| args.map? | Object |  | scope/instance map, alternative way to specify multiple scope/instances |
| args.addBRefs? | string |  | Space separated list of additional bRefs you want to load |
| args.bindingSetId? | string | bcd_docUpload | Optional binding set id used for document storage, by default bcd_docUpload is used |
| args.doAcknowledge? | string | false | Set to true if acknowledge mode is required |
| args.downloadAll? | string | false | Set to true if you want to be able to download/zip all documents |
...
| args.zipName? | string | documents.zip | The filename for the generated zip, ensure it ends with .zip |
| args.zipDownloadTargetHtml? | targetHtmlRef |  | optional targetHml htmlElement or jQuery object for zip download button, if not present, it's rendered to its default place |
| args.renderChain? | chainDef |  | A custom renderer chain |
| args.renderParameters? | Object |  | Renderer parameters. Will be enrichted with docUploader default parameters |

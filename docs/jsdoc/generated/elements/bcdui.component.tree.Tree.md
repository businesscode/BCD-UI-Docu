...
| args.config? | bcdui.core.DataProvider | "./treeConfiguration" | The model containing the tree configuration data. If it is not present a SimpleModel with the url  './treeConfiguration.xml' is created. |
| args.statusModel? | bcdui.core.DataProvider | bcdui.wkModels.guiStatus | StatusModel (default is 'guiStatus'), containing the filters as /SomeRoot/f:Filter and used to store tree expand/collapse status |
| args.id? | string |  | The object's id, needed only when later accessing via id. |
| args.persistent? | boolean | true | Tree expand/collapse status is stored |
| args.contextMenu? | (boolean\|string) | false | If true, tree's default context menu is used, otherwise provide the url to your context menu xslt here. |
...
| args.parameters? | Object |  | An object, where each property holds a DataProvider, used as a transformation parameters. |

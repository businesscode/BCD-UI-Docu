# Class Grid
<span hidden class='htmlPackage'>bcdui.component.grid</span>
...
| args.inputModel? | bcdui.core.DataProvider |  | WRS or GridModel which is used, if not provided, it is generated out of the config. If provided, config is ignored unless it is set explicitly |
...
| args.hotArgs? | Object |  | Arguments which are extended to handsontable creation |
| args.tooltipChain? | (string\|chainDef) |  | To overwrite default tooltip chain. An empty string will disable tooltips, otherwise the default gridTooltip.xslt is used |
| args.contextMenu? | (boolean\|string) | false | If true, grid's default context menu is used, otherwise provide the url to your context menu xslt here. |
...
| args.customSave? | function |  | Custom save function |
| args.afterAddRow? | function |  | Custom function(args) which is called after a row was added (args.rowNode, wrs row node which was added, args.headerMeta wrs header object) |
| args.saveChain? | chainDef |  | A chain definition which is used for the grid saving operation |
| args.saveParameters? | Object |  | Parameters for the saving chain |
| args.loadChain? | chainDef |  | A chain definition which is used for the grid loading operation |
| args.loadParameters? | Object |  | Parameters for the loading chain |
| args.validationChain? | chainDef |  | A chain definition which is used for the validation operation. basic wrs and reference validation is given by default |
| args.validationParameters? | Object |  | Parameters for the validation chain |
| args.allowNewRows? | boolean | true | Allows inserting new rows via default contextMenu or drag/paste |
| args.columnFilters? | boolean | false | Enable basic column filter input fields |
| args.maxHeight? | integer |  | Set a maximum vertical size in pixel (only used when no handsontable height is set) |
| args.isReadOnly? | boolean | false | Turn on viewer-only mode |
| args.topMode? | boolean | false | Add/save/restore buttons appear at the top, pagination at bottom, insert row at top |
| args.forceAddAtBottom? | boolean | false | Always add a new row at the bottom, no matter if topMode or pagination |
| args.disableDeepKeyCheck? | boolean | false | Set this to true if you really want to disable the deep key check which is active if your grid is only a subset of the underlying table |
| args.ignoreKeyCase? | boolean | false | Set this to true if the key test should not be case sensitive |
| args.isReadOnlyCell? | function |  | Custom check function if a given cell is read only or not. Function gets gridModel, wrsHeaderMeta, rowId, colId and value as input and returns true if the cell becomes readonly |
| args.columnFiltersGetCaptionForColumnValue? | function |  | Function which is used to determine the caption values for column filters. You need to customize this when you're e.g. using XML data in cells. |
| args.columnFiltersCustomFilter? | Object |  | CustomColumnFilter functions passed to column filter |
| args.defaultButtons? | boolean | true | Set to false if you want to hide the default buttons reset/delete/save |
| args.serverSidedPagination? | boolean | false | Set to true if you want to enable server sided pagination |
| args.paginationSize? | integer | 20 | Set pagination page size (and enable pagination) |
| args.paginationAllPages? | boolean | false | Set pagination show all option (and enable pagination) |
| args.requestPostChain? | chainDef |  | The definition of the transformation chain |
| args.modelUrl? | (string\|bcdui.core.DataProvider) | 'WrsServlet' | This is a string or string- DataProvider with the URL which to send the requestModel result to |

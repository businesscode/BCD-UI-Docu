...
| args.targetModel | bcdui.core.DataProvider |  | The model to be transformed and replaced |
| args.parameters? | Object |  | An object, where each property holds a DataProvider, used as a transformation parameters.<br/>Once this ModelUpdater is [executed](bcdui.core.AbstractExecutable#execute.md), it will check each parameter and execute it, if it is not [.isReady()](bcdui.core.AbstractExecutable#isReady.md) |
| args.autoUpdate? | boolean | true | A boolean value indicating if the ModelUpdater should run on every change in the targetModel. Can be a data modification event or if targetModel again reaches the ready status. If autoUpdate is false a model updater only runs when the targetModel is (re)executed. |

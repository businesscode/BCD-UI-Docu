...
| args.targetHtml | targetHtmlRef |  | The socrecard renderer's target html element. |
| args.inputModel? | bcdui.core.DataProvider |  | The data of the scorecard model [bcdui.component.scorecard.ScorecardModel](bcdui.component.scorecard.ScorecardModel.md). Provide args.config or args.inputModel |
| args.config? | bcdui.core.DataProvider |  | The data of the scorecard configuration model according to XSD http://www.businesscode.de/schema/bcdui/scorecard-1.0.0<br/>   If given, an internal ScorecardModel based on the configuration data will be created. args.config it will be provided as 'scConfig' to the renderer chain. Provide args.config or args.inputModel |
| args.id? | string |  | The id of the new object. If omitted the id is automatically generated. |
...
| args.tooltipUrl? | string |  | To overwrite default renderer xslt of the tooltip. An empty string will disable tooltips. <br/>   Default is BCD-UI's default sc tooltip, which shows all attributes of a cell. To give a KPI an attribute, nest an scc:AspectRef into scc:AspectKpi in the scorecard definition. |
| args.statusModel? | bcdui.core.DataProvider | bcdui.wkModels.guiStatusEstablished | StatusModel, containing the filters at /SomeRoot/f:Filter, default is 'guiStatusEstablished' |
| args.customParameter? | bcdui.core.DataProvider |  | Custom parameters for usage in custom aggregators, aspects and renderer as 'customParameter' parameter.</li> |
...
| args.contextMenu? | (boolean\|string) | false | If true, scorecard's default context menu is used, otherwise provide the url to your context menu xslt here. |

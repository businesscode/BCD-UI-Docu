...
| args.callback | function |  | The callback providing the data |
| args.doAllwaysRefresh? | boolean | false | If true, each getData() calls the callback, otherwise only execute() will do. |
...
**Returns** {string}: The text returned by the callback function
